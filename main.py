...
while True:
    ...
    while True:
        if resposta.status_code == 200 and resposta_ticket.status_code == 200:
            ...
            print(
                f"Retorno da requisição do cliente: {resposta.status_code} - {resposta.reason}"
            )
            print(
                f"Retorno da requisição do chamado: {resposta_ticket.status_code} - {resposta_ticket.reason}"
            )
            resposta, resposta_ticket = request()
    ...
    try:
        ...
        dados_filtrados["Razão Social"] = dicionario["resposta"]["data"][0][
            "name"
        ].strip()
        ...
        if resultado:
            ...
            conta_editada = partes[1] + partes[2]
            sequencias = [
                f"{conta_editada}{i:02}" for i in range(1, numero_terminais + 1)
            ]
    ...
    dados_filtrados_reorganizado = {
        "Chamado": str(dados_filtrados["Chamado"])
        + " - "
        ...
        "Endereco": str(dados_filtrados["Endereco"] or "").upper()
        + ", "
        + str(dados_filtrados["Numero"] or "").upper(),
        "Bairro": str(dados_filtrados["Bairro"]).upper(),
        "Cidade": str(dados_filtrados["Cidade"]).upper(),
        "Contato": str(dados_filtrados["COMERCIAL - Contato"]).upper(),
    ...
    tipo_mensagem_letra = (
        "já existia" if os.path.exists(subpasta_letra) else "foi criada"
    )
    ...
    tipo_mensagem_razao = (
        "já existia" if os.path.exists(pasta_razao_social) else "foi criada"
    )
    os.makedirs(pasta_razao_social, exist_ok=True)
    print(
        f"Pasta da Razão Social '{dados_filtrados['Razão Social']}' {tipo_mensagem_razao}: {pasta_razao_social}"
    )
    ...
    gerar_planilha_estilizada(
        dados_filtrados_reorganizado, arquivo_excel, caminho_imagem
    )
